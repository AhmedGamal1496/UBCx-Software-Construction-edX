...
public class BusFareCard {
    ...
    public void purchaseAdultFare() throws IllegalAgeException, NoBalanceException {
        ...
        if (ownerAge < AGE_CUTOFF) {
            throw new IllegalAgeException("This age is young!");
        }
        else if (balance < 0) {
            throw new NoBalanceException("There is no enough money");
        }
        else {
            balance -= ADULT_FARE;
            fareLoaded = true;
    ...
    public void purchaseConcessionTicket() throws IllegalAgeException, NoBalanceException {
        //TODO: complete the implementation of this method
        if (ownerAge > AGE_CUTOFF) {
            throw new IllegalAgeException("This age is old!");
        }
        else if (balance < 0) {
            throw new NoBalanceException("There is no enough money");
        }
    ...
    public void reloadBalance(double amount) throws IllegalAmountException {
        //TODO: complete the implementation of this method
        if (amount < 0) {
            throw new IllegalAmountException("That is illegal amount of money!");
        }
        else {
            balance += amount;
        }
